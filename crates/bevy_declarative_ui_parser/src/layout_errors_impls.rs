...
impl<'a> LayoutReader<'a> {
    ...
    pub(super) fn err_expected_value(&self, tag: &XmlTag) -> XmlLayoutError {
        XmlLayoutError::ExceptedValue {
            context: self.context(tag.span(), self.location(), tag.location(), tag.identifier().len()),
        }
    }
    ...
    pub(super) fn err_empty_attribute(&self, tag: &XmlTag, attribute_name: &'static str) -> XmlLayoutError {
        ...
        XmlLayoutError::EmptyAttribute {
            context: self.context(tag.span(), attribute.location(), attribute.location(), attribute_name.len() + 3),
    ...
    pub(super) fn err_missing_attribute(&self, tag: &XmlTag, attribute_name: &'static str) -> XmlLayoutError {
        XmlLayoutError::MissingAttribute {
            context: self.context(tag.span(), tag.location(), tag.location(), tag.identifier().len()),
            attribute: attribute_name,
        }
    }

    pub(super) fn err_missing_parameter(&self, value: &Value, binding_type: &str, parameter_name: &'static str) -> XmlLayoutError {
        XmlLayoutError::MissingParameter {
            context: self.context(value.span(), value.location(), value.location(), binding_type.len()),
            name: parameter_name.to_string()
        }
    }

    pub(super) fn err_unknown_binding_type(&self, value: &Value, binding_name: &str) -> XmlLayoutError {
        XmlLayoutError::UnknownBindingType {
            context: self.context(value.span(), value.location(), value.location(), binding_name.len()),
            name: binding_name.to_string()
        }
    }

    pub(super) fn err_duplicate_param(&self, source: &Value, duplicates: &[Value], parameter: &str) -> XmlLayoutError {
    ...
    fn attribute_error_list(&self, source: &Value, values: &[Value]) -> Vec<SimpleErrorSpan> {
        ...
        values.iter().for_each(|value| {
            ...
            let start = offset + value.location.column() - (value.span.start() - value.location.position());
            list.push(SimpleErrorSpan { start, length: value.span.len()});
        });

        list
    }

    fn context(&self, span: Span, location: Location, error_location: Location, length: usize) -> ErrorContext {
        ErrorContext::new(
            self.file().to_string(),
            location,
