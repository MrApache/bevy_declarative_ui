use std::fmt::Debug;
use crate::values::bindings::{format_path, BindingMode};
...
impl Params for BaseParams {
    ...
    {
        let target = Some(raw.try_take("Target")
...
impl Params for AdditionalParams {
    ...
    where
        ...
        B: Clone + Debug + PartialEq + Params
    {
        let mode = BindingMode::from_str(&raw.try_take_value("Mode").unwrap_or("ReadOnce".to_string()));
